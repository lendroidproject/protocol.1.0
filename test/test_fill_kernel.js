...
contract("Protocol - fill_kernel", function(addresses) {
  beforeEach(async function() {
    ...
    // position terms
    this.position_lending_currency_fill_value = web3.utils.toWei("10", "ether");
    this.position_borrow_currency_fill_value = web3.utils.toWei("1.1", "ether");
  ...
  it("fill_kernel should work as expected", async function() {
    ...
    tx = this.LendToken.mint(this.lender, web3.utils.toWei("100", "ether"), { from: addresses[0] });
    await mineTx(tx);
    ...
    tx = this.BorrowToken.mint(this.borrower, web3.utils.toWei("10", "ether"), { from: addresses[0] });
    await mineTx(tx);
    ...
    assert.isTrue(
      protocolContractBorrowTokenBalance.toString() === "0",
      "protocolContract's BorrowToken balance should be 0"
    );
    ...
    assert.isTrue(
      protocolContractBorrowTokenBalance.toString() === this.position_borrow_currency_fill_value,
      `protocolContract's BorrowToken balance should be ${this.position_borrow_currency_fill_value}`
    );
    borrowerLendTokenBalance = await this.LendToken.balanceOf(this.borrower);
    assert.isTrue(
      borrowerLendTokenBalance.toString() === this.position_lending_currency_fill_value,
    ...
    assert.isTrue(
      wranglerProtocolTokenBalance.toString() === this.kernel_monitoring_fee,
      `wrangler's protocolToken balance should be ${this.kernel_monitoring_fee}`
    );
    relayerProtocolTokenBalance = await this.protocolToken.balanceOf(this.relayer);
    assert.isTrue(
      relayerProtocolTokenBalance.toString() === this.kernel_relayer_fee,
      `relayer's protocolToken balance should be ${this.kernel_relayer_fee}`
    );
  });
});
