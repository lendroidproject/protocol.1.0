...
contract("Protocol - escape_hatch_token", function(addresses) {
  ...
  it("protocol contract should not accept ether", async function() {
    assert.isTrue(
      web3.utils.toDecimal(await web3.eth.getBalance(this.protocolContract.address)) === 0,
      `protocol contract's initial balance should be 0 ether`
    );
    let errr = false;
    try {
  ...
  it("tokens should be wihdrawable under an escape-hatch condition", async function() {
    ...
    assert.isTrue(
      tokenBalance.toString() === "2000000000000000000",
      `maliciousUserAddress should have a token balance of 2 TYT`
    );
    tx = maliciousToken.transfer(this.protocolContract.address, web3.utils.toWei("2", "ether"), {
      from: maliciousUserAddress,
    });
    await mineTx(tx);
    ...
    assert.isTrue(
      tokenBalance.toString() === "2000000000000000000",
      `protocolContract should have a token balance of 2 TYT`
    );
    ...
    assert.isTrue(
      tokenBalance.toString() === "2000000000000000000",
      `protocolContract owner should have a token balance of 2 TYT`
    );
  });
});
