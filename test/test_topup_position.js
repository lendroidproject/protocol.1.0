...
contract("Protocol - topup_position", function(addresses) {
  ...
  it("topup_position should not be callable by lender", async function() {
    let errr = false;
    try {
      await this.protocolContract.topup_position(this.position_hash, this.borrow_currency_topup_value, {
        from: this.lender,
      });
    } catch (e) {
      errr = true;
  ...
  it("topup_position should be callable by borrower before position has expired", async function() {
    if (!((await blockTimestamp(web3)) > this.position[8].toNumber())) {
      let errr = false;
      try {
        await this.protocolContract.topup_position(this.position_hash, this.borrow_currency_topup_value, {
          from: this.borrower,
  ...
  it("topup_position should not be callable by borrower after position has expired", async function() {
    ...
    while (!((await blockTimestamp(web3)) > this.position[8].toNumber())) {
      console.log(`Current blocktimestamp: ${await blockTimestamp(web3)}. Will check after 1s ...`);
  ...
  it("topup_position should increment borrow_currency_current_value correctly", async function() {
    if (!((await blockTimestamp(web3)) > this.position[8].toNumber())) {
      ...
      assert.isTrue(
        borrow_currency_value.toNumber() === borrow_currency_current_value.toNumber(),
        `borrow_currency_current_value should be ${borrow_currency_value}, NOT ${borrow_currency_current_value}`
      );
      let tx = await this.protocolContract.topup_position(this.position_hash, this.borrow_currency_topup_value, {
        from: this.borrower,
      });
      await mineTx(tx);
      this.position = await this.protocolContract.position(this.position_hash);
      ...
      assert.isTrue(
        borrow_currency_current_value.toNumber() === expected_borrow_currency_current_value.toNumber(),
        `borrow_currency_current_value should be ${expected_borrow_currency_current_value.toNumber()}, NOT ${borrow_currency_current_value.toNumber()}`
      );
    }
  });
});
